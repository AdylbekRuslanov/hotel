...
if(isset($_POST['cancel'])){
   ...
   if($verify_booking->rowCount() > 0){
      ...
      $success_msg[] = 'Бронирование отменено успешно!';
   }else{
      $warning_msg[] = 'Бронирование уже отменено!';
   }
...
<section class="bookings">
   ...
   <div class="box">
      ...
      <p>номер : <span><?= $fetch_booking['number']; ?></span></p>
      <p>дата приезда : <span><?= $fetch_booking['check_in']; ?></span></p>
      <p>дата отьезда : <span><?= $fetch_booking['check_out']; ?></span></p>
      <p>комнаты : <span><?= $fetch_booking['rooms']; ?></span></p>
      <p>взрослый : <span><?= $fetch_booking['adults']; ?></span></p>
      <p>детский : <span><?= $fetch_booking['childs']; ?></span></p>
      <p> id брони : <span><?= $fetch_booking['booking_id']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="booking_id" value="<?= $fetch_booking['booking_id']; ?>">
         <input type="submit" value="Отмена брони" name="cancel" class="btn" onclick="return confirm('cancel this booking?');">
      </form>
   </div>
   <?php
    }
   ...
   <div class="box" style="text-align: center;">
      <p style="padding-bottom: .5rem; text-transform:capitalize;">Брони не найдены!</p>
