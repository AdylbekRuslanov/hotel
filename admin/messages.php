...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Отзыв удален!';
   }else{
      $warning_msg[] = 'Отзыв уже удален!';
   }
...
<section class="grid">
   ...
   <div class="box">
      ...
      <p>номер : <span><?= $fetch_messages['number']; ?></span></p>
      <p>отзыв : <span><?= $fetch_messages['message']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="delete_id" value="<?= $fetch_messages['id']; ?>">
         <input type="submit" value="удалить отзыв" onclick="return confirm('Удалить этот отзыв?');" name="delete" class="btn">
      </form>
   </div>
   <?php
      }
