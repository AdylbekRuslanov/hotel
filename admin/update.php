...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      ...
      if($verify_name->rowCount() > 0){
         $warning_msg[] = 'Уже существует такой user!';
      }else{
         $update_name = $conn->prepare("UPDATE `admins` SET name = ? WHERE id = ?");
         $update_name->execute([$name, $admin_id]);
         $success_msg[] = 'User обновлен!';
      }
   }
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         $warning_msg[] = 'Старый пароль не соответсвует!';
      }elseif($c_pass != $new_pass){
         $warning_msg[] = 'Новый пароль не соответствует!';
      }else{
         if($new_pass != $empty_pass){
            ...
            $warning_msg[] = 'Пожалуйста введите новый пароль!';
         }
      }
   }
